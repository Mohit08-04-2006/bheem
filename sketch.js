...
function setup(){
  ...
  // Creating sprites
  background2 = createSprite(width-500,height-290,1200,400);
  background2.addImage(backgroundImg);
  background2.scale = 2.2;

  gameover = createSprite(width/2,height-340,20,20);
  gameover.addImage(gameoverImg);
  
  restart = createSprite(width-630,height-300,20,20);
  restart.addImage(restartImg);
  restart.scale = 0.15;
  
  ground = createSprite(width-1000,height-100,600,20);
  ground.visible = false;
  
  bheem = createSprite(width-1150,height-120,30,40);
...
function draw(){
  ...
  fill("red");
  textSize(30);
  text("score = "+score,width-1200,20);
  text("life = "+life,width-160,20);

  if (score% 8 == 0){
    background2.velocityX = background2.velocityX - 2;
  }
...
function spawnLadoos(){
  if(frameCount% 80 == 0){
  var s = Math.round(random(height/2,height-260));
  ladoo = createSprite(width-50,s,20,20);
  ladoo.addImage(ladooImg);
  ladoo.scale = 0.06;
  ladoo.velocityX =-(8+score/4);
  ladoo.lifetime = 200;
